...
#ifdef EMPL_NO_64BIT
    gyro[0] =
        (long)(((float)gyro[0] * 65536.f) / test.gyro_sens / packet_count);
    gyro[1] =
        (long)(((float)gyro[1] * 65536.f) / test.gyro_sens / packet_count);
    gyro[2] =
        (long)(((float)gyro[2] * 65536.f) / test.gyro_sens / packet_count);
    if (has_accel) {
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug) {
    ...
    float st_shift_cust[3], st_shift_ratio[3], ct_shift_prod[3],
        accel_offset_max;
